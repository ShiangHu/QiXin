...
%%%%function varargout=catscatter(y,x,jitter,style)
% Stand-alone version of notBoxPlot from Rob Campbell with some 
% modifications, including 'mean' and 'meadian' styles.
%
% June 16, 2016 Lucas Parra
%
...
if isvector(y) && isvector(x) && length(x)>1
    ...
    for ii=1:length(u)
        f=x==u(ii);
        h(ii)=catscatter(y(f),u(ii),jitter,style);
    end
...
function h=myPlotter(X,Y)
 ...
 SEM= nanstd(Y)./sqrt(sum(~isnan(Y)))*1.96; % SME 5%-95% confidence interval
 SD=nanstd(Y);  %Requires the stats toolbox 
 mu=nanmean(Y); %Requires the stats toolbox 

 if strcmp(style,'median'), mu=nanmedian(Y); end
 ...
 for k=1:length(X)
     ...
     if strcmp(style,'mean') || strcmp(style,'median')
